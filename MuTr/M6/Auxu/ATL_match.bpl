...
procedure T2TB_matchAll() returns ();
 ...
 read($srcHeap, t1, HSM$Transition.source) == src && read($srcHeap, t1, HSM$Transition.target) == trg && read($srcHeap, c, HSM$AbstractState.compositeState) == src && !(dtype(trg) == HSM$CompositeState) && false  ==>
...
{  read(hp, t1, HSM$Transition.source) == src && read(hp, t1, HSM$Transition.target) == trg && read(hp, c, HSM$AbstractState.compositeState) == src && !(dtype(trg) == HSM$CompositeState) && false  }
