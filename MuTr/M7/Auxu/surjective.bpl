...
t1!=null && read($s, t1, alloc) && dtype(t1) <: HSM$Transition 
 ...
 &&  read($s, t1, HSM$Transition.source) == src && read($s, t1, HSM$Transition.target) == trg && read($s, c, HSM$AbstractState.compositeState) == trg && !(dtype(src) == HSM$CompositeState) && false  &&
  getTarsBySrcs(Seq#Build(Seq#Build(Seq#Build(Seq#Singleton(t1),src),trg),c)) == t2)
