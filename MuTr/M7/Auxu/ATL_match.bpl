...
procedure T2TC_matchAll() returns ();
 ...
 read($srcHeap, t1, HSM$Transition.source) == src && read($srcHeap, t1, HSM$Transition.target) == trg && read($srcHeap, c, HSM$AbstractState.compositeState) == trg && !(dtype(src) == HSM$CompositeState) && false  ==>
getTarsBySrcs(Seq#Build(Seq#Build(Seq#Build(Seq#Singleton(t1),src),trg),c)) ==null || !read($tarHeap, getTarsBySrcs(Seq#Build(Seq#Build(Seq#Build(Seq#Singleton(t1),src),trg),c)), alloc)); 
  modifies $tarHeap,$linkHeap;
// Rule Outcome
  ensures (forall t1,src,trg,c: ref ::
...
{  read(hp, t1, HSM$Transition.source) == src && read(hp, t1, HSM$Transition.target) == trg && read(hp, c, HSM$AbstractState.compositeState) == trg && !(dtype(src) == HSM$CompositeState) && false  }
