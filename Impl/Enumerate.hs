...
data ProveTree = 
  Tree Option [Expression] ProveTree |                                
  Node Option Expression [Expression] |                                
  Err
...
myEnumerate (Poss expr poss) op = case (myEnumerate expr op) of
  Node real_expr_op real_expr real_expr_poss -> case length real_expr_poss of
    0 -> Node real_expr_op real_expr [GenBy real_expr pos | pos <- poss]  
    otherwise -> Tree real_expr_op [GenBy expr pos | pos <- poss] (myEnumerate expr op)
  Tree real_expr_op real_expr_poss tree -> Tree real_expr_op [GenBy expr pos | pos <- poss] (myEnumerate expr op)
  otherwise -> Err
...
myEnumerate (UnaryExpression uop expr) op = case (myEnumerate expr op) of
  Node real_expr_op real_expr real_expr_poss -> case length real_expr_poss of
    0 -> Node (flipOption real_expr_op) (UnaryExpression uop real_expr) []
    otherwise -> Node (flipOption real_expr_op) (UnaryExpression uop real_expr) [(UnaryExpression uop real_expr_pos) | real_expr_pos <- real_expr_poss]
  Tree real_expr_op real_expr_poss tree -> Tree (flipOption real_expr_op) [(UnaryExpression uop real_expr_pos) | real_expr_pos <- real_expr_poss] (myEnumerate expr op)
  otherwise -> Err
...
myEnumerate (Fcall "allInstance" exprs) op =  case length exprs of
  1 -> Node op (Fcall "allInstance" exprs) []
  otherwise -> Err
